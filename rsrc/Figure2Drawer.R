...
scores <- read.csv(file = "2.withCalibration/2.R_pXg/PeptideAnnotationS2_5ppm_002_recal.rep1.rank10.pXg.fdr.dist", header = T, sep = "\t", as.is = as.double())
allResult <- read.csv(file = "2.withCalibration/2.R_pXg/PeptideAnnotationS2_5ppm_002_MSFrecal.tsv", header = T, sep = "\t", as.is = as.double())
...
ggsave("score.all.S2.png", plot = g, width = 10, height = 8, units = "in", dpi = 300)
...
ggsave("score.canonical.S2.png", plot = g, width = 10, height = 8, units = "in", dpi = 300)
...
g <- ggplot(data = nc_scoreTable, aes(x=Score, y=Count, fill=Class)) +
  ...
  labs(y="PSMs", x = "Score")
ggsave("score.noncanonical.S2.png", plot = g, width = 10, height = 8, units = "in", dpi = 300)

## RNA-read distrubition
data <- read.csv(file = "2.withCalibration/2.R_pXg/PeptideAnnotationS2_5ppm_002_recal.rep1.rank10.pXg.pval.dist", header = T, sep="\t", as.is = as.double())
...
ggsave("read.S2.png", plot = g, width = 10, height = 8, units = "in", dpi = 300)

## PPM-tolerance
ppmData <-  read_excel(path = "2.withCalibration/Laumont_Results.xlsx", sheet = "subject2")
allData <- read.csv(file = "2.withCalibration/2.R_pXg/PeptideAnnotationS2_5ppm_002_MSFrecal.tsv", header = T, sep = "\t", as.is = as.double())
...
#cPPM <- cPPM[grep("rep6_021213_Fx50mM", cPPM$`Source File`), ]$ppm
cPPM <- cPPM$ppm
ncPPM <- ppmData[ppmData$IsCanonical == F, ]$ppm
ncFDRPPM <- ppmData[ppmData$IsCanonical == F, ]
ncFDRPPM <- ncFDRPPM[ncFDRPPM$`Denovo score` >= 82, ]$ppm
...
data$Class <- factor(data$Class, c("All PSMs", "cPSMs", "ncPSMs", "ncPSMs (FDR < 0.05)"))
...
ggsave("ppm.S2.png", plot = g, width = 10, height = 8, units = "in", dpi = 300)
...
t.test(data[data$Class == "All PSMs", ]$`PPM tolerance`, 
       data[data$Class == "cPSMs", ]$`PPM tolerance`, paired = F, alternative = "two.sided")
