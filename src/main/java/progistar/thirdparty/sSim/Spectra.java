...
public class Spectra {
	...
	public Spectra (String fileName, int fileType, ArrayList<Integer> selectiveScans) {
		...
		this.fileType = fileType;
		if(this.fileType == FILE_TYPE_MGF) {
			readMGF (this.fileName, selectiveScans);
		}
	}
	...
	public Spectra (String fileName, int fileType) {
		...
		sortedRTSpectra = new TreeMap<>();
		scanIndexer = new Hashtable<>();
		if(this.fileType == FILE_TYPE_MGF) {
			try {
			...
			try {
				...
				Hashtable<Double, String> peakDupCheck = new Hashtable<>();
				int peakMax =-1;
				while((line = BR.readLine()) != null) {
					if(line.startsWith("Num peaks:")) {
						peakMax = Integer.parseInt(line.split("\\:")[1].trim());
						peaks = new ArrayList<>();
						peakDupCheck = new Hashtable<>();
						index ++;
		...
		try {
			...
			while((line = BR.readLine()) != null) {
				if(line.startsWith("BEGIN")) {
					...
					spectra.add(spectrum);
					if(sortedRTSpectra.get(rt) != null) {
						System.err.println("Duplicated RT was observed!");
					}
	...
	public ArrayList<Spectrum> getSpectraByRT (double from, double to) {
		ArrayList<Spectrum> rtSpectra = new ArrayList<>();
	...
	public void writeToMGF (String fileName, int[] msLevels) throws IOException {
		...
		for(int i=0; i<size; i++) {
			...
			if(spectrum != null) {
				...
				for(int targetMSLevel : msLevels) {
					if(msLevel == targetMSLevel) {
						isTargeted = true;
					}
				}
