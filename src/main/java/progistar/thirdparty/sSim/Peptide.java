...
public class Peptide {
	...
	public Peptide getDecoyPeptide () {
		...
		for(int i=1; i<sequence.length()+1; i++) {
			decoyPeptide.modifications[i] = this.modifications[sequence.length()-i+1];
		}
	...
	public double[] getTheoreticalLadder (double ion, double charge, boolean withMods) {
		...
		if(withMods && nTermMods != null) {
			for(Modification mod : nTermMods) {
				nTermModi += mod.getModMass();
			}
		}
		...
		// charge and ion
		for(int i=0; i<sequenceLength; i++) {
			ladder[i] = (ladder[i] + (charge-1)*ProteomeConstants.Proton + ion) / charge;
		}
	...
	public void addModification (Modification mod, int position) {
		if(this.modifications[position] == null) {
			this.modifications[position] = new ArrayList<>();
		}
	...
	public double getTheoreticalMass (boolean withMods) {
		...
		for(int i=0; i<sequence.length(); i++) {
			theoreticalMass += AminoAcid.getAminoAcid(sequence.charAt(i)).getResidualMass();
		}
		if(withMods) {
			for(ArrayList<Modification> mods : this.modifications) {
				if(mods != null) {
					for(Modification mod : mods) {
						theoreticalMass += Double.parseDouble(mod.modMass);
					}
				}
			}
		}
