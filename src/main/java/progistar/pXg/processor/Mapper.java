...
public class Mapper {
	...
	public static void gMap (GenomicSequence genomicSequence, int start,
		...
		Hashtable<Integer, Boolean> txdIndexer = new Hashtable<>();
		...
		for(Cigar cigar : genomicSequence.cigars) {
	    	...
	    	case 'M': // match or mismatch
	    		for (int relativePosition : cigar.relativePositions) {
	    			txdIndices = genomicAnnotationIndex[gRelPos + relativePosition];
	    			for(int txdIndex : txdIndices) {
						txdIndexer.put(txdIndex, true);
					}
	    		}
	    		break;
