...
public class GenomicAnnotation {
	...
	private Hashtable<Integer, TBlock> idToTx = new Hashtable<>();
	// txid to index id
	private Hashtable<String, Integer> txToId = new Hashtable<>();
	// transcripts
	private ArrayList<TBlock> tBlocks = new ArrayList<>();
	...
	public void assignTypesInTBlocks () {
		...
		if(idToTx.size() != 0) {
			idToTx.forEach((k, v) -> {v.assignBlockTypes();});
		}
	...
	public int[][] getIndexingBlocks (int chrIndex, int start, int end) {
		...
		for(int i=0; i<sizeOfTBlocks; i++) {
			...
			// chr selection
			// out of range
			if((chrIndex != tBlock.chrIndex) || (tStart > end)) {
				continue;
			}
