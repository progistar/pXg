...
public class Q3_0_Synthetic {
	...
	public static void main(String[] args) throws IOException {
		...
		Hashtable<String, Spectra> syntheticSpectra = new Hashtable<>();
		...
		while(peptides.hasNext()) {
			...
			for(Target target : targets_) {
				...
				for(int i=0; i<size;i ++) {
					...
					if((spectrum.getCharge() == 0) || (spectrum.getCharge() != target.spectrum.getCharge())) {
						continue;
					}
	...
	public static Hashtable<String, ArrayList<Target>> getSyntheticList (File file, File[] mgfs) throws IOException {
		ArrayList<Spectra> specSets = new ArrayList<>();
		for (File mgf : mgfs) {
			if(mgf.getName().startsWith("THP1_2") ||
					mgf.getName().startsWith("THP1_3") ||
					mgf.getName().startsWith("B_LCL1") ||
					mgf.getName().startsWith("B_LCL2") ||
					mgf.getName().startsWith("B_LCL3") ||
					mgf.getName().startsWith("B_LCL4")) {
				System.out.println(" read file: "+mgf.getAbsolutePath());
			}
			specSets.add(new Spectra(mgf.getAbsolutePath(), Spectra.FILE_TYPE_MGF));
		}

		Hashtable<String, ArrayList<Target>> targets = new Hashtable<>();
